...
// FFmpeg's HEVC decoder produces 8-bit and 10-bit planar formats. We support
// the same set as H264DecoderImpl to minimise surprise (4:2:0/4:2:2/4:4:4 in
// 8-bit and 10-bit) — Main and Main10 land in 4:2:0 8-bit / 4:2:0 10-bit,
// respectively; the others are tolerated for Range Extensions streams.
...
bool H265DecoderImpl::Configure(const Settings& settings) {
  ...
  // HEVC SW is heavier than H.264; thread_count > 1 is a known perf win on
  // 1080p30, but breaks the buffer pool single-thread invariant. Stay at 1
  // until Phase 5 perf tuning rebuilds the pool for thread-safe callbacks.
  ...
  if (!codec) {
    ...
    RTC_LOG(LS_ERROR) << "FFmpeg HEVC decoder not found.";
    Release();
    ReportError();
    return false;
  }
...
int32_t H265DecoderImpl::Decode(const EncodedImage& input_image,
  ...
  std::optional<int> qp = h265_bitstream_parser_.GetLastSliceQp();
