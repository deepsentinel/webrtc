...
// flag has on the HEVC decoder or avcodec_send_packet, though, so this is in
// some way depending on undocumented behavior. If any problems turn up, we
// may have to add an extra copy operation, to enforce padding before buffers
// are passed to ffmpeg.
extern "C" {
...
class H265DecoderImpl : public H265Decoder {
 public:
  ...
  // `missing_frames` and `render_time_ms` are ignored.
 ...
 private:
  ...
  // Used by ffmpeg via `AVGetBuffer2()` to allocate I420/I010/I422/I444 images.
  VideoFrameBufferPool ffmpeg_buffer_pool_;
  std::unique_ptr<AVCodecContext, AVCodecContextDeleterH265> av_context_;
  std::unique_ptr<AVFrame, AVFrameDeleterH265> av_frame_;
